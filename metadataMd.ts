...
const wsProvider = new WsProvider(
  Deno.env.get("PROVIDER") ?? "wss://rpc.polkadot.io",
);
console.log(1);
...
function renderPage(page: Page): string {
  ...
  page.sections.forEach((section) => {
    md += `- **[${stringCamelCase(section.name)}](#${
      stringCamelCase(section.name).toLowerCase()
    })**\n\n`;
  });
  ...
  page.sections.forEach((section) => {
    ...
    section.items.forEach((item) => {
      ...
      Object.keys(item).filter((key) => !["link", "name"].includes(key))
        .forEach((bullet) => {
          md += `\n- **${bullet}**: ${
...
function sortByName<T extends { name: Codec | string }>(a: T, b: T): number {
  ...
  return a.name.toString().toUpperCase().localeCompare(
...
function addRpc(): string {
  const sections = Object
    .keys(definitions)
    .filter((key) =>
      Object.keys(definitions[key as "babe"].rpc || {}).length !== 0
    );
      ...
      .reduce((all: Section[], _sectionName): Section[] => {
        ...
        Object.keys(section.rpc)
          ...
          .forEach((methodName) => {
            ...
            const topName = method.aliasSection
              ? `${_sectionName}/${method.aliasSection}`
              : _sectionName;
            ...
            const args = method.params.map(
              ({ isOptional, name, type }: DefinitionRpcParam): string => {
          ...
          items: lookup.getSiType(calls.unwrap().type).def.asVariant.variants
            ...
            .map(({ docs, fields, name }, index) => {
              ...
              const args = fields.map(({ name, type }) =>
                `${name.isSome ? name.toString() : `param${index}`}: ` + "`" +
...
function addEvents({ lookup, pallets }: MetadataLatest): string {
  return renderPage({
    ...
    sections: pallets
      ...
      .map((meta) => ({
        items: lookup.getSiType(meta.events.unwrap().type).def.asVariant
          .variants
          .sort(sortByName)
          .map(({ docs, fields, name }) => {
            ...
            return {
              interface: "`" +
                `api.events.${stringCamelCase(meta.name)}.${methodName}.is` +
                "`",
              name: `${methodName}(${args})`,
              ...(docs.length && { summary: docs }),
            };
...
function addErrors({ lookup, pallets }: MetadataLatest): string {
  return renderPage({
    ...
    sections: pallets
      ...
      .map((moduleMetadata) => ({
        items: lookup.getSiType(moduleMetadata.errors.unwrap().type).def
          .asVariant.variants
          .sort(sortByName)
          .map((error) => ({
            interface: "`" +
              `api.errors.${
                stringCamelCase(moduleMetadata.name)
              }.${error.name.toString()}.is` + "`",
